...
int AnalysisThread37::Initialize(int nchan) {
  ...
  std::cout << "AnalysisThread37: Number of histograms created: "
            << nhist << std::endl;
  return nhist;
}


int AnalysisThread37::Analyze() {
  // Read SIMDAQ
  double* ADC1;
  int* CH1;
  qint64* TS1;
  ...
  QPair<int, double*> pADC1 = ReadData<double>("A36","ADCOutput");
  nADC1 = pADC1.first;
  ADC1 = pADC1.second;
  QPair<int, int*> pCH1 = ReadData<int>("A36","CHAN");
  nCH1 = pCH1.first;
  CH1 = pCH1.second;
  QPair<int, qint64*> pTS1 = ReadData<qint64>("A36","TS");
  nTS1 = pTS1.first;
  TS1 = pTS1.second;
  ...
  for (int i = 0; i < nADC1; ++i) {
    ...
    if (GetHistogram(histname)) {
      ...
      std::cerr << "AnalysisThread37::Analyze: ADC1 channel out of range!  ch="
    << CH1[i] << std::endl;
    }
  }
